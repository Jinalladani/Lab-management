...
const OTP_KEY = "forgot_password_otp";
...
const ForgotPasswordReset = () => {
  ...
  }, [navigate, storedEmail, storedOtp]);
  ...
  const validateForm = () => {
    ...
    if (!password.trim()) newErrors.password = "Password is required";
      ...
      newErrors.password = `Password must be at least ${MIN_PASSWORD_LEN} characters`;
    }

    if (!confirm.trim()) newErrors.confirm_password = "Please confirm your password";
    else if (confirm !== password) newErrors.confirm_password = "Passwords do not match";
  ...
  const handleReset = async (e) => {
    ...
    try {
      ...
      await forgotPasswordReset(storedEmail, storedOtp, formData.password);
      ...
      if (error.response?.data?.message) {
        setErrors((prev) => ({ ...prev, common: error.response.data.message }));
      } else {
        setErrors((prev) => ({
          ...prev,
  ...
  const inputClass =
    "w-full bg-transparent outline-none border-none text-[14px] md:text-[15px] text-[#585050]/80 placeholder:text-[#585050]/55 pl-3";

  const inputWrapper =
    "relative h-[48px] md:h-[52px] border-[3px] border-white rounded-[8px] shadow-[0_4px_4px_rgba(0,0,0,0.15)] flex items-center pl-[46px] pr-10 bg-white/40";
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-3/5 bg-gradient-to-br from-[#2562AA] to-[#1e4f8a] flex-col justify-center items-center text-white p-12 relative overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          {/* <p className="text-xl mb-8 font-light tracking-wide">Reset Password</p> */}
          ...
          <p className="text-xl opacity-90 max-w-lg mx-auto leading-relaxed mb-8">
            Create a new secure password for your laboratory management account
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 lg:w-2/5 flex items-center justify-center p-6 lg:p-8 bg-white">
        <div className="w-full max-w-sm">
          ...
          <div className="w-full text-center text-sm text-gray-600 mb-6">
            ...
            Updating password for <span className="font-bold">{storedEmail}</span>
          </div>
