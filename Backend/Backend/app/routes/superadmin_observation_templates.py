...
def ensure_observation_template_storage():
    ...
    statements = [
        ...
        """,
        """
        CREATE TABLE IF NOT EXISTS observation_template_materials (
        ...
        """,
        """
        CREATE TABLE IF NOT EXISTS observation_template_tests (
        ...
        """,
        """
        CREATE TABLE IF NOT EXISTS observation_template_standards (
        ...
        """,
        """
        CREATE UNIQUE INDEX IF NOT EXISTS uq_observation_template_materials_name_active
            ON observation_template_materials (lower(name))
            WHERE deleted_at IS NULL
        """,
        """
        CREATE UNIQUE INDEX IF NOT EXISTS uq_observation_template_tests_name_active
            ON observation_template_tests (lower(name))
            WHERE deleted_at IS NULL
        """,
        """
        CREATE UNIQUE INDEX IF NOT EXISTS uq_observation_template_standards_name_active
            ON observation_template_standards (lower(name))
            WHERE deleted_at IS NULL
        """,
        """
        ALTER TABLE observation_templates
        ...
        """,
        """
        CREATE TABLE IF NOT EXISTS observation_template_builder_data (
        ...
        """,
        """
        INSERT INTO observation_template_materials (name, created_by)
        SELECT DISTINCT trim(material), 'migration'
        FROM observation_templates
        WHERE material IS NOT NULL AND trim(material) <> ''
        ON CONFLICT DO NOTHING
        """,
        """
        INSERT INTO observation_template_tests (name, created_by)
        SELECT DISTINCT trim(test), 'migration'
        FROM observation_templates
        WHERE test IS NOT NULL AND trim(test) <> ''
        ON CONFLICT DO NOTHING
        """,
        """
        INSERT INTO observation_template_standards (name, created_by)
        SELECT DISTINCT trim(standard), 'migration'
        FROM observation_templates
        WHERE standard IS NOT NULL AND trim(standard) <> ''
        ON CONFLICT DO NOTHING
        """,
        """
        UPDATE observation_templates ot
        SET material_id = m.material_id
        FROM observation_template_materials m
        WHERE ot.material_id IS NULL
          AND ot.material IS NOT NULL
          AND lower(trim(ot.material)) = lower(m.name)
          AND m.deleted_at IS NULL
        """,
        """
        UPDATE observation_templates ot
        SET test_id = t.test_id
        FROM observation_template_tests t
        WHERE ot.test_id IS NULL
          AND ot.test IS NOT NULL
          AND lower(trim(ot.test)) = lower(t.name)
          AND t.deleted_at IS NULL
        """,
        """
        UPDATE observation_templates ot
        SET standard_id = s.standard_id
        FROM observation_template_standards s
        WHERE ot.standard_id IS NULL
          AND ot.standard IS NOT NULL
          AND lower(trim(ot.standard)) = lower(s.name)
          AND s.deleted_at IS NULL
        """,
        """
        INSERT INTO observation_template_builder_data (template_id)
        SELECT template_id
        FROM observation_templates
        WHERE deleted_at IS NULL
